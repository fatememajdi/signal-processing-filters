...
def iir_filter(data, cutoff, fs, btype='low', order=5):
    nyquist = 0.5 * fs
...
# Apply the IIR filters
low_passed_data = iir_filter(data, low_cutoff, fs, btype='low', order=order)
high_passed_data = iir_filter(data, high_cutoff, fs, btype='high', order=order)

# For band-pass, we need to design the filter differently
nyquist = 0.5 * fs
band_normal_cutoff = [c / nyquist for c in band_cutoff]
b_band, a_band = butter(order, band_normal_cutoff, btype='band', analog=False)
band_passed_data = lfilter(b_band, a_band, data)

# Write the filtered data to new .wav files
wav.write('Iir_output_low_pass.wav', fs, low_passed_data.astype(np.int16))
wav.write('Iir_output_high_pass.wav', fs, high_passed_data.astype(np.int16))
wav.write('Iir_output_band_pass.wav', fs, band_passed_data.astype(np.int16))

# Play the filtered audio files
print("Playing Low-Pass Filtered Audio...")
play_wav_file('Iir_output_low_pass.wav')

print("Playing High-Pass Filtered Audio...")
play_wav_file('Iir_output_high_pass.wav')

print("Playing Band-Pass Filtered Audio...")
play_wav_file('Iir_output_band_pass.wav')
